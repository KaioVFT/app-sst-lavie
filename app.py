...
def render_acomp_gerar_pdf():
    ...
    if modulo_selecionado != "Selecione...":
        ...
        # INÍCIO DA MODIFICAÇÃO (Opções lado a lado e Lote por Obra)
        st.markdown("<br>", unsafe_allow_html=True)
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("<h4 style='color:#E37026; font-size: 1rem;'>Gerar PDF Individual</h4>", unsafe_allow_html=True)
            registro_selecionado = st.selectbox("SELECIONE O CADASTRO", opcoes_registros, label_visibility="collapsed")
            btn_gerar_individual = st.button("RECONSTRUIR PDF ÚNICO", use_container_width=True)
        ...
        # Lógica PDF Individual (Seu código original encapsulado na função DRY)
        if btn_gerar_individual and registro_selecionado != "Selecione um registro para gerar o PDF...":
            ...
            with st.spinner("A reconstruir o PDF individual com os dados da nuvem..."):
                try:
                    # Chamei uma função oculta que criei abaixo para não poluir esta tela com o "if nome_aba == EPI..." 7 vezes.
                    pdf_final_bytes, nome_arquivo_final = _processar_linha_pdf(nome_aba, linha)
                except Exception as e:
                    st.error(f"Erro ao processar PDF: {e}")

        # Nova Lógica: PDF em Lote
        elif btn_lote:
            with st.spinner(f"A compilar lote de PDFs para '{obra_selecionada}'... Pode demorar alguns segundos."):
                try:
                    ...
                    for linha in registros[1:]:
                        ...
                        # Manda o objeto FPDF para a função ir adicionando páginas nele, em vez de criar um novo
...
# Criada para não repetir o if/elif gigantesco
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def _processar_linha_pdf(nome_aba, linha, pdf_batch=None):
    ...
    if nome_aba == "EPI":
        ...
        # O parâmetro pdf_batch diz se é pra criar um PDF novo ou colar na página do lote
